...
function isIndividualEmailError(error) {
  const individualErrorCodes = [
    "RCPT_TO_FAILED", // Recipient failed
  ...
  return (
    ...
    errorString.includes("550") || // Mailbox unavailable
    errorString.includes("551") || // User not local
    errorString.includes("552") || // Mailbox full
...
async function processEmailBatch(job, releaseNote, transport) {
  ...
  for (const emailRow of emailRows) {
    try {
      ...
      if (isIndividualEmailError(error)) {
        // Log individual email failure but continue
        failedEmails.push({
  ...
  if (failedEmails.length > 0) {
    console.log(`Batch failed emails:`, failedEmails);
  }
...
export async function POST(request) {
  try {
    ...
    // Structure the data like individual email system
    const releaseNote = {
    ...
    try {
      ...
      // Infrastructure/server error - pause job and schedule retry
      const retryCount = job.retry_count + 1;
      const retryDelay = Math.min(Math.pow(2, retryCount) * 60000, 30 * 60000); // Exponential backoff, max 30 minutes
      const nextRetryAt = new Date(Date.now() + retryDelay);

      await mysql.execute(
        "UPDATE kics_email_jobs SET status = ?, retry_count = ?, next_retry_at = ?, error_message = ? WHERE id = ?",
        ["paused", retryCount, nextRetryAt, error.message, job.id]
      );

      console.error(`Job ${job.id} paused due to error:`, error);

      return Response.json({
        success: false,
        jobId: job.id,
        error: "Job paused due to server error",
        retryAt: nextRetryAt,
      });
    }
  } catch (error) {
